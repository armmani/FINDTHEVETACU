...
export default function VetAppointmentPage() {
  ...
  const load = async () => {
    ...
    setApt(data)
    setOwnerName((data?.owner as any)?.full_name || '')
    setLoading(false)
  }
  ...
  return (
    <div className="max-w-lg mx-auto h-[calc(100vh-7rem)] flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-4 shrink-0">
        <Link href="/vet/dashboard" className="text-gray-400 hover:text-gray-600">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="font-bold text-lg">แชทกับเจ้าของ</h1>
      ...
      <div className="card mb-3 shrink-0 text-sm space-y-1.5">
        <div className="flex items-center gap-2 text-gray-700">
          ...
          <span className="text-gray-400">· {apt.pet_type} · {apt.pet_age}</span>
        </div>
        ...
        {apt.symptoms && (
          <p className="text-gray-500 pt-1 border-t border-gray-100">อาการ: {apt.symptoms}</p>
        )}
      </div>
