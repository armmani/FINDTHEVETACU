...
function RegisterForm() {
  ...
  const handleGoogleRegister = async () => {
    if (!role) { toast.error('กรุณาเลือกก่อนว่าคุณเป็นใคร'); return }
    if (!pdpaAccepted) { toast.error('กรุณายอมรับนโยบายความเป็นส่วนตัวก่อน'); return }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gray-50">
      <div className="card w-full max-w-sm">
        ...
        {/* Role selector */}
        <p className="text-sm text-gray-500 text-center mb-2">คุณเป็นใคร?</p>
        ...
        <button
          ...
          disabled={googleLoading || !role || !pdpaAccepted}
          className="w-full flex items-center justify-center gap-3 py-2.5 px-4 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors text-sm font-medium text-gray-700 disabled:opacity-40 disabled:cursor-not-allowed mb-4"
        >
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <button type="submit" disabled={loading || !pdpaAccepted || !role} className="btn-primary w-full py-2.5 disabled:opacity-50">
