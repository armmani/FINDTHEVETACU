...
/** ลิสต์คำขอเชื่อมสัตว์เลี้ยงที่รออนุมัติ — ใช้ฝังท้ายหน้าตรวจสอบ (เช่น /admin/verify) */
export default function PetOwnershipRequestList() {
  ...
  const handleDecision = async (req: Request, decision: 'approved' | 'rejected') => {
    ...
    if (req.pets) {
      await supabase.from('notifications').insert({
        ...
        title: decision === 'approved' ? `✅ อนุมัติการเชื่อมสัตว์เลี้ยง "${req.pets.name}"` : `❌ ปฏิเสธการเชื่อมสัตว์เลี้ยง "${req.pets.name}"`,
        body: note || null,
        link: decision === 'approved' ? '/owner/pets' : '/owner/pets/claim',
      })
    }
  ...
  return (
    <div>
      <h2 className="font-bold text-lg mb-3 flex items-center gap-2">
        <PawPrint className="w-5 h-5 text-primary-600" /> คำขอเชื่อมสัตว์เลี้ยง ({requests.length})
      </h2>
      {requests.length === 0 ? (
